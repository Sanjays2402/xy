...
def test_chart_gallery_grid_renders_every_type_as_inline_svg(
    ...
    assert len(chart_section) == 19
    assert "XYChart" not in rendered
    assert rendered.count("dangerouslySetInnerHTML") == 33
    ...
    assert rendered.count("aspect-[320/232]") == 33
    assert rendered.count("shadow-large") == 33
    assert rendered.count("transition-bg") == 33
    ...
    assert rendered.count("size:14") == 33
...
def test_inline_svg_gallery_validator_requires_every_styled_preview(tmp_path: Path) -> None:
    ...
    module_path.write_text(
        preview * 32 + "gallery-preview-surface aspect-[320/232] shadow-large",
        encoding="utf-8",
    )
    with pytest.raises(RuntimeError, match="32 previews, expected 33"):
        check_html_routes.validate_inline_svg_gallery("/overview/gallery/", module_path)
